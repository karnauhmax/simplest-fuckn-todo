...
- **2026-08-07 — the deployment lives in Frankfurt, not Washington: `vercel.json` pins `regions: ["fra1"]` and Atlas M0 goes to AWS `eu-central-1`.** The user is in Amsterdam; Vercel's default `iad1` would have put every API call across the Atlantic and, worse, split the function from the database. Hobby allows exactly one function region, so this is a choice, not a spread — and the two must be changed together: an Atlas cluster moved without changing `regions` reintroduces the split silently, with nothing failing to show it.
- **2026-08-07 — `pnpm-lock.yaml` deleted; npm is the package manager.** A lockfile last written in part 4 sat next to a current `package-lock.json`, and Vercel picks pnpm when it sees one — then fails on `--frozen-lockfile`, because six dependencies had been added through npm since. Regenerating the pnpm lockfile instead would leave two lockfiles to keep in step, and every script, the README and CI already speak npm. Nothing local changes; `npm ci` was already the install path.
- **2026-08-07 — TypeScript pinned back to the 5.x line (5.9.3) because Vercel compiles the API functions with the project's own TypeScript.** `typescript@7` is the native port: it exports two symbols and has no `ts.sys`, so `@vercel/node` died with `Cannot read properties of undefined (reading 'readFile')` — after Vite had already built the client, which is why the log looks like a success followed by a non sequitur. Keeping 7 and having Vercel use something else is not offered by the builder, and precompiling the handlers to JS ourselves would trade a one-line pin for a build step. Cost: no native-port speed locally until `@vercel/node` supports TS 7.
